...
def sample_fn(generator,
    ...
    generated_image = tokenizer.decode_tokens(
        generated_tokens.view(generated_tokens.shape[0], -1), qzshape
    )
    if return_tensor:
        return generated_image

    generated_image = torch.clamp(127.5 * generated_image + 128.0, 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()
