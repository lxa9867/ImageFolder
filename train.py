...
def build_everything(args: arg_util.Args):
    ...
    vae_local, var_wo_ddp = build_vae_var(
        V=args.codebook_size, Cvae=args.codebook_embed_dim, ch=160, share_quant_resi=4,        # hard-coded VQVAE hyperparameters
    ...
    vae_ckpt = args.vae_ckpt # '/sensei-fs/users/xiangl/output/exp45/2024-09-15-15-33-50/000-VQ-16/checkpoints/best_ckpt.pt'
    ...
    del checkpoint
    if start_ep == 0:
        vae_local.load_state_dict(model_weight, strict=True)
    ...
    print(f'[INIT][#para] ' + ', '.join([f'{k}={count_p(m)}' for k, m in (('VAE', vae_local), ('VAE.enc', vae_local.encoder), ('VAE.dec', vae_local.decoder), ('VAE.quant', vae_local.quantizes))]))
    ...
    if trainer_state is not None and len(trainer_state):
        trainer.load_state_dict(trainer_state, strict=False, skip_vae=False) # don't load vae again
...
def main_training():
    ...
    if dist.get_rank() == 0:
        wandb_tracker = wandb.init(project='VAR', )
    # train
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            val_loss_mean, val_loss_tail, val_acc_mean, val_acc_tail, tot, cost, chw = trainer.eval_ep(ld_val)
            if dist.get_rank() == 0:
                wandb_tracker.log({
                    "val_L_mean": val_loss_mean,
                    "val_L_tail": val_loss_tail,
                    "val_acc_mean": val_acc_mean,
                    "val_acc_tail": val_acc_tail,
                    "cost": cost,
                }, step=ep)
                wandb_tracker.log({"recon_images": [wandb.Image(chw)]}, step=ep)
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        ...
        # trainer.var.module.p_drop *= (1 - ep/args.ep)  # set p_drop
        grad_norm, scale_log2 = trainer.train_step(
            it=it, g_it=g_it, stepping=stepping, metric_lg=me, tb_lg=tb_lg,
            inp_B3HW=inp, label_B=label, prog_si=prog_si, prog_wp_it=args.pgwp * iters_train, p_drop_factor=(1 - ep/args.ep),
        )
