...
from tokenizer.tokenizer_image.cliploss import ClipLoss
...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.semantic_guide = 'dinov2' # 'none' #
    ...
    def forward(self, input):
        ...
        if self.semantic_guide != 'none':
            ...
            z_q_ = torch.mean(quant, dim=(2, 3)).contiguous()
            z_s = self.sem_linear(z_s).contiguous()
            sem_loss = self.semantic_loss.forward(z_s, z_q_, logit_scale=self.sem_loss_scale)
            sem_loss = sem_loss * self.sem_loss_weight
        else:
            sem_loss = None
        return dec, diff, sem_loss
