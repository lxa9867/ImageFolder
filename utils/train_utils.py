...
from RAR.modules import ARLoss
# from RAR.titok import TiTok, PretrainedTokenizer
...
def create_pretrained_tokenizer(config, accelerator=None):
    pretrianed_tokenizer = VQModel(ModelArgs(**config.model.vq_model))
    if config.model.vq_ckpt:
        checkpoint = torch.load(config.model.vq_ckpt, map_location="cpu")
        pretrianed_tokenizer.load_state_dict(checkpoint["model"])
        if config.model.ema:
            pretrianed_tokenizer.load_state_dict(checkpoint["ema"])
